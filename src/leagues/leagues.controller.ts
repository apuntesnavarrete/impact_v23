...
@Get()
async allTeams(): Promise<Leagues[]>{
...
@Get(':id')
async findTeamById(@Param('id' , ParseIntPipe) id : number): Promise<Leagues[]>{
    if (isNaN(id)) {
...
async create(
    ...
    @Body() teamsData:Partial<Leagues>): Promise<Leagues>{
...
@Put(':id')
 async updateTeam(@Param('id') id : number ,@Body() teamsData:Partial<Leagues>): Promise <UpdateResult>{
    if (isNaN(id)) {
        // Lanza una excepción BadRequest si 'id' no es un número válido
        throw new HttpException('El ID proporcionado no es un número válido.', HttpStatus.BAD_REQUEST);
      }
